...
PolarCode::PolarCode(int& n, int& K, BscChannel& channel)
{
	...
	//îïðåäåëÿåì, êàêèå áèòû ÿâëÿþòñÿ èíôîðìàöèîííûìè, à êàêèå-çàìîðîæåííûìè
	construct();
}
...
{
	std::vector<int> temp(n_+1);
	std::vector<double> temp2(n_+1);
...
//Ðåàëèçóåò ìåòîä BEC ïîëÿðíîãî ïîñòðîåíèÿ. 
//Èíèöèàëèçèðóåò ïåðâûé êàíàë ñ âåðîÿòíîñòüþ ñòèðàíèÿ ðàâíîé BER âìåñòî 0,5
void PolarCode::dependent_bec_construction()
{
	//âû÷èñëÿåì Z-ïàðàìåòð äëÿ êàæäîãî èç N ïîëÿðèçîâàííûõ êàíàëîâ
	for (int i = 0; i < N_; i++) {
	...
	//ñîðòèðóåì âñå êàíàëû â ïîðÿäêå âîçðàñòàíèÿ èõ Z-ïàðàìåòðà, òàê êàê ÷åì ìåíüøå Z-ïàðàìåòð, 
	//òåì áîëåå ïîäõîäÿùèì äëÿ ïåðåäà÷è èíôîðìàöèè ÿâëÿåòñÿ ýòîò êàíàë
...
//Ðåêóðñèâíî âû÷èñëÿåòñÿ çíà÷åíèå Z - ïàðàìåòðà Áõàòòà÷àðüè äëÿ i - ãî èç N ïîëÿðèçîâàííûõ êàíàëîâ.
//Ðàñ÷åò îñíîâàí íà ñîîòíîøåíèÿõ, ïîëó÷åííûõ â îðèãèíàëüíîé ðàáîòå Àðèêàíà.
//èñïðàâèòü -> 1 ðàç ñ÷èòàòü (z)^2
...
//Ñèñòåìàòè÷åñêîå êîäèðîâàíèå
//Êîäèðóåò K èíôîðìàöèîííûõ áèòîâ â N áèòîâ ñîîáùåíèÿ êîäîâîãî ñëîâà ïóòåì çàïîëíåíèÿ
//èõ ñ çàìîðîæåííûìè áèòàìè è ñ ïîìîùüþ ïîëÿðíîãî ïðåîáðàçîâàíèÿ ïîñëå ýòîãî
...
void PolarCode::extend_info_bits(std::vector<int>& info_bits)
{
	if (info_bits.size() != K_) {
		std::cout << "Íå óäàëîñü çàêîäèðîâàòü ñîîáùåíèå. Íåêîððåêòíàÿ äëèíà êîäèðóåìîãî ñîîáùåíèÿ.";
		return;
	}
	...
	// óñòàíîâèì çíà÷åíèÿ èíôîðìàöèîííûõ áèòîâ â u_message
	//(íà ïîçèöèè info_bits_positions_ óñòàíàâëèâàåì çíà÷åíèÿ info_bits)
	for (int i = 0; i < K_; i++) {
...
//Ðåàëèçóåò ïîëÿðíîå ïðåîáðàçîâàíèå äàííîãî ñîîáùåíèÿ ðåêóðñèâíûì ñïîñîáîì
